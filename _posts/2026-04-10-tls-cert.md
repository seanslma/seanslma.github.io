...
TLS is not authorization: A valid TLS certificate proves who the server is (Authentication) and ensures no one can listen in (encryption). It does not determine what a user is allowed to do (authorization).
...
- **Validation**: The client verifies the certificate's metadata: the domain name matches, the expiration date is valid, and the digital signature is from a trusted Certificate Authority (CA).
...
Adding a section on **TLS Interception** (often referred to as **SSL Inspection** or **HTTPS Inspection**) is crucial for a complete note, as it represents the only common scenario where the "End-to-End" encryption model is intentionally modified.

Here is a technically refined version of that section, formatted to match the style of your previous notes.
